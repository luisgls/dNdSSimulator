...
We now can also plot the results using the following function to visualize the clonal structure over time based on the acquisition of driver events, we can also track the global dN/dS obtained by taking all allive cells at each time point:
...
Another function to visualize dN/dS over time:

```{r visualize2}

#Visualize  dN/dS over time
dnds_plot(M)


```

We can aggregate the values corresponding to the major clones on the population and see how their dN/dS fluctuate over time

```{r}
#Visualize dN/dS over time using major clones
plot_clone_dnds(M)
```
...
Here, each of the single simulations can be plotted as shown before, but now we are more interested on the global behavior of multiple simulations over time. So we can use the following functions to summarise data over time. 
...
```
The first function tell us the dN/dS with the confidence interval by taking all mutations present in alive cells at each time point

```{r getseq}
all_genotypes<-lapply(RUNS,get_sequencing)

calc_freq_dnds(RUNS,all_genotypes, freq = 0.05)
```
Here, you have a summary for each simulation, the number of the simulation, the last population size recorded, the time at which the last population size was recorded, the different nonsynonymous and synonymous mutations, the global, driver and immune dN/dS, and the cut-off frequency. 
