...
simulator = function(list_params)
  ...
  source("/home/haem/lortiz/tools/dNdSSimulator/functions_ZCsimulator.R")
  ...
  for (i in 1:GENERATIONS) {
    ...
    immunogenic_clones = immunogenic_cloneid(M, i)
    table_immunogenic = count_immunogenic_cells(M , i)
    ...
    if (length(immunogenic_clones) > 0 & (pattack > runif(1))) {
      ...
      print(paste(icells,
                  paste(
                    paste("killed in Immunogenic clone",immunogenic_clones,sep=" "), paste("at time", i, sep=" ")
                  )
                  ,sep=" "))
      M = M %>%
        mutate(alive = ifelse(
