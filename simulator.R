...
simulator = function(list_params)
  ...
  for (i in 1:GENERATIONS) {
    ...
    #Kill all clones wit immunogenic mutations
    M = set_immunedead3(M,i,pattack)
...
plotter = function(j, M) 
  ...
  # Visualization
  if (count(M %>% select(clone_id) %>% unique()) > 1) {
    p1 <-
      mullerplot(
        M,
        t_from = 1,
...
stats_dnds <- function(x)
  {
  fulltimeseries<-tibble( t_0 =NA, median_dnds=NA, median_dndsi=NA, median_dndsd=NA, 
                          dnds=NA, dnds_lci=NA,dnds_hci=NA, 
                          dnds_i=NA,dnds_ilci=NA,dnds_ihci=NA, 
                          dnds_d=NA,dnds_dlci=NA,dnds_dhci=NA, 
                          count=NA, expon=NA, sum_na=NA, sum_ns=NA, 
                          gnad=NA, gnap=NA, gnai=NA, gnae=NA, gnak=NA,
                          gns=NA, gnsi=NA, gnsd=NA, numclones=NA)
  ...
  for (i in 1:length(x)){
    ...
    DNDS = M %>% 
      ...
      mutate(dnds = compute_gdnds(gnad,gnap,gnai,gnae,gnak,gns,gnsi,gnsd),
             dnds_i = compute_gdnds_i(gnai,gnsi),
             dnds_d = compute_gdnds_d(gnad,gnsd),
             dnds_lci=  dnds - (1.96*(sqrt(dnds/(gnad+gnap+gnai+gnae+gnak+gns+gnsi)))),
             dnds_hci=  dnds + (1.96*(sqrt(dnds/(gnad+gnap+gnai+gnae+gnak+gns+gnsi)))),
             dnds_ilci= dnds_i - (1.96*(sqrt(dnds_i/(gnai+gnsi)))),
             dnds_ihci= dnds_i + (1.96*(sqrt(dnds_i/(gnai+gnsi)))),
             dnds_dlci= dnds_d - (1.96*(sqrt(dnds_d/(gnad+gnsd)))),
             dnds_dhci= dnds_d + (1.96*(sqrt(dnds_d/(gnad+gnsd))))
      )
    ...
    time_series = full_join(pop, DNDS) %>% full_join(totals) %>%
      select(t_0, median_dnds, median_dndsi, median_dndsd, 
             dnds, dnds_lci,dnds_hci, 
             dnds_i,dnds_ilci,dnds_ihci, 
             dnds_d,dnds_dlci,dnds_dhci, 
             count, expon, sum_na, sum_ns, 
             gnad, gnap, gnai, gnae, gnak, gns, gnsi, gnsd, numclones)
...
##Obtain summary statistics table for collapse, extinction and ongoing
evodevo = function(x,min,max) 
  {
  fullevolution<-tibble(simulation_number=NA,time=NA,event=NA,fin_dnds=NA,fin_dndsd=NA,fin_dndsi=NA)
  ...
  for (i in 1:length(x)){
    ...
    M = M %>% mutate(t_0 = ifelse(is.na(t_0), max(M$t_0)+1, t_0) )
    #M = M %>% filter(strategy != "Death")
    popsize<-nrow(M %>% filter(alive,t_0 == max(M %>% select(t_0))))
    lasttime<-max(M %>% select(t_0))
    finaldnds<- M %>% filter(t_0==(lasttime)) %>% summarise(finaldnds= (sum(gnad)+sum(gnap)+sum(gnai)+sum(gnae)+sum(gnak)) / (3*(sum(gns)+sum(gnsi)+sum(gnsd))) ) %>% pull(finaldnds) 
    finaldndsd<- M %>% filter(t_0==(lasttime)) %>% summarise(finaldndsd= (sum(gnad)) / (3*(sum(gnsd))) ) %>% pull(finaldndsd) 
    finaldndsi<- M %>% filter(t_0==(lasttime)) %>% summarise(finaldndsi= (sum(gnai)) / (3*(sum(gnsi))) ) %>% pull(finaldndsi) 
    counts <-  M %>% filter(t_0==(lasttime)) %>% summarise(counts = sum(gnad)) %>% pull (counts)
    counts2 <-  M %>% filter(t_0==(lasttime)) %>% summarise(counts2 = sum(gnai)) %>% pull (counts2)
    ...
    evolution<-tibble(simulation_number=i,
                      time=lasttime, 
                      event=ifelse(popsize > max, "Collapse", ifelse( popsize < min, "Extinction", "Ongoing")), 
                      fin_dnds=finaldnds,
...
dnds_time = function(x,min,max) 
  {
  fullevolution<-tibble(simulation_number=NA,time=NA,event=NA)
  fullevolution<-fullevolution[complete.cases(fullevolution),]
  ...
  for (i in 1:length(x)){
    ...
    popsize<-nrow(M %>% filter(alive,t_0 == max(M %>% select(t_0))))
    lasttime<-max(M %>% select(t_0))
    finaldnds<- M %>% group_by(t_0) %>% summarise(simulation_number=i,finaldnds= (sum(gnad)+sum(gnap)+sum(gnai)+sum(gnae)+sum(gnak)) / (3*(sum(gns)+sum(gnsi)+sum(gnsd))) ) %>% pull(finaldnds) 
    finaldndsd<- M %>% group_by(t_0) %>% summarise(simulation_number=i,finaldndsd= (sum(gnad)) / (3*(sum(gnsd))) ) 
...
plotter2 = function(x, name)
  ...
  p1<-ggstatsplot::ggbetweenstats(
    ...
    messages = FALSE
  ) + geom_abline(slope = 0,intercept = 1, colour ="red") #+  scale_fill_manual(values = colorRampPalette(brewer.pal(12, "Accent"))(colourCount)) 
  ...
  p3<-ggstatsplot::ggbetweenstats(
    ...
    package = "ggsci", # package from which color palette is to be taken
    palette = "default_jama", # choosing a different color palette
    xlab = "Generation",                       # label for the x-axis variable
    messages = FALSE
  ) #+  scale_fill_manual(values = colorRampPalette(brewer.pal(12, "Accent"))(colourCount)) 
  
  p4<-ggstatsplot::ggbetweenstats(
    data = fulltimeseries %>% filter(sum_ns > 0) %>% mutate(norm=log(dnds)/(count)),
    x = t_e, 
    y = norm,
    #grouping.var = immune,
    #pairwise.comparisons = TRUE,      # display significant pairwise comparisons
  ...
  finalp1
  ggsave(paste("simul_",name, ".pdf" ,sep = ""), scale= 4, width = 30, height = 25, units = "cm")
...
plot_five = function (x) 
  ...
  p1 <- ggboxplot(x,
                  x = "t_0",
                  y = "dnds",
                  color = "#4DBBD5FF", palette = "jco") +
    geom_abline(slope = 0,intercept = 1, col = "red") + 
    labs(title = "Overall selection over time", y = 'dN/dS', x = "Generation") + 
    theme_minimal() + geom_text_repel(data = subset(medians, dnds<1) 
                                      , aes(label = dnds, x = t_0-1), 
                                      nudge_y = -0.5, 
                                      nudge_x = 0.5, 
                                      direction = "y") +
    geom_text_repel(data = subset(medians, dnds>=1) 
                    , aes(label = dnds, x = t_0-1), 
                    nudge_y = 0.5, 
                    nudge_x = 0.5, 
                    direction = "x")
  ...
  p2 <- ggboxplot(x,
                  ...
                  y = "dnds_d",
                  color = "#4DBBD5FF", palette = "jco") +
    geom_abline(slope = 0,intercept = 1, col = "red") + labs(title = "Driver selection over time", y = 'dN/dS', x = "Generation") + 
    theme_minimal()+ geom_text_repel(data = subset(medians, dnds_d<1) 
                                     , aes(label = dnds_d, x = t_0-1), 
                                     nudge_y = -0.5, 
                                     nudge_x = 0.5, 
                                     direction = "y") +
    geom_text_repel(data = subset(medians, dnds_d>=1) 
                    , aes(label = dnds_d, x = t_0-1), 
                    nudge_y = 0.5, 
                    nudge_x = 0.5, 
                    direction = "x")
  ...
  p3 <- ggboxplot(x,
                  ...
                  y = "dnds_i",
                  color = "#4DBBD5FF", palette = "jco") +
    geom_abline(slope = 0,intercept = 1, col = "red") + labs(title = "Immune selection over time", y = 'dN/dS', x = "Generation") + 
    theme_minimal() + geom_text_repel(data = subset(medians, dnds_i<1) 
                                      , aes(label = dnds_i, x = t_0-1), 
                                      nudge_y = -0.5, 
                                      nudge_x = 0.5, 
                                      direction = "y") +
    geom_text_repel(data = subset(medians, dnds_i>=1) 
                    , aes(label = dnds_i, x = t_0-1), 
                    nudge_y = 0.5, 
                    nudge_x = 0.5, 
                    direction = "x")
  ...
  p4 <- ggboxplot(x,
                  x = "t_0",
                  y = "count",
                  color = "#00A087FF", palette = "jco") +
    geom_abline(slope = 0,intercept = 32,col = "red") +  labs(title = "Population size", y = 'Pop size', x = "Generation") + 
    theme_minimal()
...
plot_three= function (x) 
  ...
  p4 <- ggboxplot(x,
                  ...
                  y = "count",
                  color = "#00A087FF", palette = "jco") +
    geom_abline(slope = 0,intercept = 32,col = "red") +  
...
plot_threeV2= function (x, subset_n = 20, maxt = 100, maintitle = "Summary simulation") 
  {
  ...
  p1 <- ggplot(x,aes(x = t_0,y = dnds, color=simulation, alpha=0.5)) +  geom_point() + 
    geom_line() +
    geom_abline(slope = 0,intercept = 0, col = "red") +  
    labs(title = "Overall selection over time", y = 'dN/dS', x = "Generation") + 
    theme_minimal() + theme(legend.position = "none") + scale_y_log10() + xlim(0,maxt)
    
  p2 <- ggplot(x,aes(x = t_0,y = count, color=simulation, alpha=0.5)) +  geom_point() + 
    geom_line() +
    labs(title = "Population size", y = 'Pop size', x = "Generation") + 
    theme_minimal() + theme(legend.position = "none") + scale_y_log10() +  geom_abline(slope = 0,intercept = log10(16),col = "red") + xlim(0,maxt)
  
  p3 <- ggplot(x,aes(x = t_0,y = sum_na, color=simulation, alpha=0.5)) +  geom_point() + 
    geom_line() +
  ...
  annotate_figure(finalp1, top = text_grob(paste("Fig",maintitle,sep=" "), color = "black", face = "bold", size = 14))
  
}

##Plot ongoing versus collapsse versus extinction for multiple simulations
plot_threeV3=function (fulltimeevo, subset_n = 50, maxt = 100, maintitle="Summary simulation")
  {
  ...
  fulltimeevo$event <- factor(fulltimeevo$event, levels = c("Ongoing","Collapse","Extinction"))
  
  p002<-ggplot(data=fulltimeevo, aes(y=time, x = event, fill=event)) + geom_violin(aes(alpha=0.5)) + 
    scale_alpha(guide = 'none')  + 
    theme_classic2() + ylim(0,maxt) + scale_fill_brewer(palette = "Dark2",drop=F) + 
    stat_summary(fun.y=mean, geom="point", shape=23, size=2) +
     stat_summary(fun.y=median, geom="point", shape=20, size=2) + 
    stat_summary(aes(label=round(..y..,2)),fun.y=median, geom="text",hjust = -1)
  
  p004<-ggplot(data=fulltimeevo, aes(x=event, fill = event)) + geom_bar(aes(alpha=0.5)) + 
    theme_classic2() + ylim(0,subset_n) + scale_fill_brewer(palette = "Dark2",drop=F)+
    geom_text(aes(label=..count..),stat="count",position=position_stack())
   
  p003<-ggplot(data=fulltimeevo, aes(fill=event, x = time))  + theme_classic2() +
    geom_dotplot(binaxis='x', 
                 stackdir = 'up', 
                 position = "dodge",
                 method = "histodot",
                 dotsize = .3, 
                 aes(fill=event)) + scale_fill_brewer(palette = "Dark2",drop=F) + xlim(0,maxt)
  
  
  p005<- ggarrange(ggarrange(p002,p004,nrow = 1,labels = c("A","B"),legend = "none"),p003,nrow=2,labels = c("","C"),  
  common.legend = T)
  
  annotate_figure(p005, top = text_grob(paste("Fig",maintitle,sep=" "), color = "black", face = "bold", size = 14))
  
}

##Plot global, driver and immune dnds for different simulations
plot_threeV4= function (x, subset_n = 20, maxt = 100, maintitle = "Summary simulation") 
  {
  ...
  #### Population count
  p1 <- ggplot(x,aes(x = t_0,y = count, color=simulation, alpha=0.5)) +  geom_point() + 
    geom_line() +
    labs(title = "Population size", y = 'Pop size', x = "Generation") + 
    theme_minimal() + theme(legend.position = "none") +  geom_abline(slope = 0,intercept = log10(16), col = "red") +
    scale_color_manual(values = mycolors)+ xlim(0,maxt) + scale_y_log10() 
  
  #### Mutation count
  p2 <- ggplot(x,aes(x = t_0,y = sum_na, color=simulation, alpha=0.5)) +  geom_point() + 
    geom_line() +
    labs(title = "Mutations", y = 'Non-silent muts', x = "Generation") + 
    theme_minimal() + theme(legend.position = "none") + scale_y_log10() + xlim(0,maxt) + 
  ...
  ###Global dN/dS
  p3 <- ggplot(x,aes(x = t_0,y = dnds, color=simulation, alpha=0.5)) +  geom_point() + 
    geom_line() +
    geom_abline(slope = 0,intercept = 0, col = "red") +  
    labs(title = "Overall selection over time", y = 'dN/dS', x = "Generation") + 
    theme_minimal() + theme(legend.position = "none") + scale_y_log10() + xlim(0,maxt) +
 ...
 ####Driver dN/dS
    dfx <- x %>% mutate(dnds_d=ifelse(is.na(dnds_d), 0, dnds_d), dnds_d=ifelse(is.infinite(dnds_d),count/16,dnds_d))
    if(nrow(dfx)>0){
      p4 <- ggplot(dfx,aes(x = t_0,y = dnds_d, color=simulation, alpha=0.5)) +  geom_point() + 
      geom_line() +
      geom_abline(slope = 0,intercept = 0, col = "red") +  
  ...
  if(nrow(dfi)>0){
    p5 <- ggplot(dfi %>% filter(!is.na(dnds_i),!is.infinite(dnds_i)),aes(x = t_0,y = dnds_i, color=simulation, alpha=0.5)) +  geom_point() + 
      geom_line() +
      geom_abline(slope = 0,intercept = 0, col = "red") +
  ...
  p6 <- ggplot(x,aes(x = t_0,y = numclones, color=simulation, alpha=0.5)) +  geom_point() + 
    geom_line() +
    labs(title = "Clone heterogeneity", y = 'Number of clones', x = "Generation") + 
    theme_minimal() + theme(legend.position = "none") +  
    scale_color_manual(values = mycolors)+ xlim(0,maxt) 
  
  finalp1 <- ggarrange(p1,p2,p6,p3,p4,p5, labels = "AUTO", common.legend = F)

  annotate_figure(finalp1, top = text_grob(paste("Fig",maintitle,sep=" "), color = "black", face = "bold", size = 14))
}

##Plot metrics from different classes
plot_metric_class= function (x, subset_n = 80, maxt = 100, maintitle = "Summary simulation") 
{
  #library(viridis)
  library("ggsci")
  library("ggplot2")
  ...
  listofsimulations<- sample(1:max(x$simulation),subset_n,replace = T)
  x<-x %>% filter(simulation %in% listofsimulations)
  ...
  p1 <- ggplot(x,aes(x = t_0,y = dnds, color=class, alpha=0.5)) +  geom_point(alpha = 0.5, size = 1) + 
    #geom_line() +
    geom_abline(slope = 0,intercept = 0, col = "red") +  
    labs(title = "Overall selection over time", y = 'dN/dS', x = "Generation") + 
    theme_minimal()  + scale_y_log10() + xlim(0,maxt) + 
    #geom_smooth(method = "loess", alpha = 0.05, size = 1, span = 1) + 
    scale_color_npg() #+
    #scale_color_manual(values = mycolors, drop =F)
  
  p2 <- ggplot(x %>% filter(t_0 <= maxt),aes(x = factor(t_0),y = count, fill=factor(class))) +  #geom_point(alpha = 0.5, size = 1) + 
    geom_boxplot(position="dodge2",outlier.shape = NA, alpha=0.5) + #geom_line() +
    labs(title = "Population size", y = 'Pop size', x = "Generation") + 
    theme_minimal()  + scale_y_log10() +  
    geom_abline(slope = 0,intercept = log10(16),col = "red") + 
    scale_fill_npg() #+
   # xlim(0,maxt) 
    #scale_color_manual(values = mycolors, drop =F)
  ...
  p3 <- ggplot(x,aes(x = t_0,y = sum_na, color=class, alpha=0.5)) +  geom_point(alpha = 0.5, size = 1) + 
    #geom_line() +
    labs(title = "Mutations", y = 'Non-silent muts', x = "Generation") + 
    theme_minimal() + scale_y_log10() + 
    xlim(0,maxt) + scale_color_npg()
    #scale_color_manual(values = mycolors, drop =F)
  
  finalp1 <- ggarrange(p2,p3,p1, nrow = 3, labels = "AUTO", common.legend = T, legend = "bottom")
  
  annotate_figure(finalp1, top = text_grob(paste("Fig",maintitle,sep=" "), color = "black", face = "bold", size = 14))
  
}

plot_metric_classevo=function (fulltimeevo, maxt = 100, maintitle="Summary simulation")
{
  ...
  p002<-ggplot(data=fulltimeevo %>% filter(event!="Ongoing"), aes(y=time, x = class, fill=class)) + geom_violin(aes(alpha=0.5)) + 
    scale_alpha(guide = 'none')  + 
    ...
    theme_classic2() + ylim(0,maxt) + scale_color_npg(alpha = 0.5) + facet_wrap(~event)
  
  p004<-ggplot(data=fulltimeevo, aes(x=class, fill = class)) + geom_bar(aes(label=..count.., alpha=0.5, fill= event),stat="count",position=position_stack()) + 
    theme_classic2() + scale_fill_nejm() #+ scale_color_nejm()
  ...
  p003<-ggplot(data=fulltimeevo, aes(fill=event, x = time, alpha =0.5))  + theme_classic2() +
    geom_dotplot(binaxis='x', 
                 ...
                 #stackgroups = T,
                 dotsize = .9, 
                 aes(fill=event)) + scale_fill_nejm() + facet_wrap(~class) +guides(alpha=F) 
  
  
  p005<- ggarrange(ggarrange(p004,p002,nrow = 1,labels = c("A","B"),legend = "none"),p003,nrow=2,labels = c("","C"),  
                   common.legend = T)
...
{
  p1 <- ggstatsplot::grouped_ggbetweenstats(data = x %>% filter(event != "Ongoing"), 
                                      x = class2,
                                      y = time,
                                      grouping.var =  event,
                                      type = "np",
                                      ...
                                      bf.message = F,
                                      results.subtitle = F,messages = F
                                      
  )
  p2 <- ggstatsplot::grouped_ggbetweenstats(data = x %>% filter(fin_dnds != Inf, event != "Ongoing"), 
                                            x = class2,
                                            y = fin_dnds,
                                            grouping.var =  event,
                                            type = "np",
  ...
  p3 <- ggstatsplot::grouped_ggbetweenstats(data = x %>% filter(fin_dndsd != Inf,event != "Ongoing"), 
                                            x = class2,
                                            y = fin_dndsd,
                                            grouping.var =  event,
                                            type = "np",
  ...
  p4 <- ggstatsplot::grouped_ggbetweenstats(data = x %>% filter(fin_dndsi != Inf,event != "Ongoing"), 
                                            x = class2,
                                            y = fin_dndsi,
                                            grouping.var =  event,
                                            type = "np",
...
plot_statistic_clasevo2 = function(x)
{
  ...
  p3 <- ggstatsplot::ggbarstats(
    data = x %>% mutate(class2 = ifelse(class == "I:0.99", "HIGH", ifelse(class == "I:0.95", "HIGH", ifelse(class == "I:0.75", "HIGH", ifelse(class == "I:0.50", "MID", "LOW"))))),
    main = class2,
    condition = event,
    sampling.plan = "jointMulti",
    perc.k = 1,
...
get_clone_tree = function(M) 
  {
  M %>% 
    select(parent_clone_id, clone_id) %>%
    distinct() %>%
...
mullerplot = function(M, t_from = 0, 
  ...
  if(nC > 20 & legend) message("More than 20 clones, if the legend is messy use `legend = FALSE`.")
  ...
  # Clone tree
  Clone_phylogeny = get_clone_tree(M %>% filter(t_0 >= t_from & t_0 <= t_to))
  # %>%
  ...
  ###Fitness
  Fitness = get_clone_fitness(M %>% filter(t_0 >= t_from & t_0 <= t_to))
  
  Pop_counts = Pop_counts %>% left_join(Fitness, by = "Identity")
  
  Muller_df <- get_Muller_df(Clone_phylogeny, Pop_counts, cutoff = cutoff)
  ...
  p3 = Muller_plot(Muller_df, colour_by = "Fitness", ...) +
    scale_fill_gradientn(colors = c('steelblue', 'darkred'), breaks = seq(0, 1, 0.1)) +
    # 
...
clonetree_plot = function(M, ps_0 = 0.5, t_to = max(M$t_0), ...) 
  {
  require(tidygraph)
  require(ggraph)
  ...
  ggraph(tb_tree, 'tree') +
    ...
    theme(legend.position = 'bottom',
          legend.key.size = unit(.3, "cm")) +
    scale_color_gradientn(colors = c('steelblue', 'darkred'), breaks = seq(0, 1, 0.1)) +
...
dnds_plot= function(M, t_to = max(M$t_0), ...)
  ...
  time_series = full_join(pop, DNDS) %>% full_join(totals) %>%
    select(t_0, median_dnds, median_dndsi, median_dndsd, 
           ...
           dnds_d,dnds_dlci,dnds_dhci, 
           count, expon, sum_na, sum_ns, 
           gnad, gnap, gnai, gnae, gnak, gns, gnsi, gnsd)
  
  #time_series = time_series %>% filter(t_0 <= 26)
  
  
  #quartz()
  p1 <- ggplot( time_series %>% 
                  filter(!is.na(dnds), dnds!=Inf),   
                aes(x = t_0, y = dnds, ymax = dnds_hci, ymin=dnds_lci)) + 
    geom_point() +
  ...
  p2<- ggplot( time_series %>% 
                 filter(!is.na(dnds_i), dnds_i!=Inf),   
               aes(x = t_0 , y = dnds_i, ymax = dnds_ihci, ymin=dnds_ilci)) +  
    geom_point() +
    geom_pointrange(col="plum4") +
    geom_line(aes(col="#c5b2c5",alpha=0.2)) +
    cowplot::theme_cowplot() +
  ...
  p3<- ggplot( time_series %>% 
                 filter(!is.na(dnds_d), dnds_d!=Inf),   
               aes(x = t_0 , y = dnds_d, ymax = dnds_dhci, ymin=dnds_dlci)) +  
    geom_point() +
    geom_pointrange(col="plum4") +
    geom_line(aes(col="#c5b2c5",alpha=0.2)) +
    cowplot::theme_cowplot() +
  ...
  else {ggarrange(p1,p3,p2,nrow = 3, legend = "none")}
}
...
plot_clone_dnds = function(x)
  ...
  data = x %>% filter(strategy != "Death") %>% 
    ...
    mutate(dnds = compute_gdnds(gnad,gnap,gnai,gnae,gnak,gns,gnsi,gnsd),
           dnds_i = compute_gdnds_i(gnai,gnsi),
           dnds_d = compute_gdnds_d(gnad,gnsd),
           escape = ifelse( (gnae/count)>0.99 , "clonal",ifelse(gnae == 0, "absent", "subclonal"))
    ) %>%
    ungroup()
  ...
  data<-data %>% mutate(dnds_d = ifelse(is.infinite(dnds_d),cells,dnds_d), dnds_d=ifelse(is.na(dnds_d), 0, dnds_d))
  ...
  p1<-  ggplot(
    data %>% filter(!is.na(dnds), dnds!=Inf), 
    aes(x = t_0, y = dnds, color = paste(clone_id))
  ) +
    geom_line() +
    xlim(c(0,max(data$t_0))) +
    geom_point(aes(size = proportion, alpha= 0.8, shape = escape)) + 
    cowplot::theme_cowplot()+ scale_alpha(guide = 'none') + geom_abline(slope = 0,intercept = 0,col = "red")   + 
    labs(title = "Overall Selection", y = 'dN/dS', x = "Generation") + scale_y_log10()+
    scale_shape_manual(name = "Escape",
  ...
  p2<-  ggplot(
    data %>% filter(!is.na(dnds_i), dnds_i!=Inf), 
    aes(x = t_0, y = dnds_i, color = paste(clone_id))
  ) +
    geom_line() +
    xlim(c(0,max(data$t_0))) +
    geom_point(aes(size = proportion, alpha= 0.8, shape = escape)) +
  ...
  p3<-  ggplot(
    data %>% filter(!is.na(dnds_d), dnds_d!=Inf), 
    aes(x = t_0, y = dnds_d, color = paste(clone_id))
  ) +
    geom_line() +
    xlim(c(0,max(data$t_0))) +
    geom_point(aes(size = proportion, alpha= 0.8, shape = escape)) +
  ...
  p4<- ggplot(
    data,
    aes(x = t_0, y = count, color = paste(clone_id))
  ) +
    geom_line() +
    xlim(c(0,max(data$t_0))) +
    geom_point(aes(size = proportion, alpha= 0.8, shape = escape)) +
    cowplot::theme_cowplot()+ scale_alpha(guide = 'none') + geom_abline(slope = 0,intercept = log10(16),col = "red")   + 
  ...
  ggarrange(p1,p2,p3,p4,nrow = 4,labels = "AUTO", common.legend = T, legend = "bottom")
